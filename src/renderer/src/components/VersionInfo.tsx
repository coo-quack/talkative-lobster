...
export function VersionInfo() {
  ...
  useEffect(() => {
    const versionPromise = window.lobster?.getAppVersion?.()
    if (versionPromise) {
      versionPromise.then(setVersion).catch(() => {})
    }

    const updatePromise = window.lobster?.checkForUpdate?.()
    if (updatePromise) {
      updatePromise.then(setUpdate).catch(() => {})
    }
  }, [])
